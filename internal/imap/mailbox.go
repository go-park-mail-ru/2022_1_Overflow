...
func (s *IMAPMailbox) Name() string

// Info returns this mailbox info.
func (s *IMAPMailbox) Info() (*imap.MailboxInfo, error)
...
func (s *IMAPMailbox) Status(items []imap.StatusItem) (*imap.MailboxStatus, error)

// SetSubscribed adds or removes the mailbox to the server's set of "active"
// or "subscribed" mailboxes.
func (s *IMAPMailbox) SetSubscribed(subscribed bool) error
...
func (s *IMAPMailbox) Check() error
...
func (s *IMAPMailbox) ListMessages(uid bool, seqset *imap.SeqSet, items []imap.FetchItem, ch chan<- *imap.Message) error

// SearchMessages searches messages. The returned list must contain UIDs if
// uid is set to true, or sequence numbers otherwise.
func (s *IMAPMailbox) SearchMessages(uid bool, criteria *imap.SearchCriteria) ([]uint32, error)
...
func (s *IMAPMailbox) CreateMessage(flags []string, date time.Time, body imap.Literal) error

// UpdateMessagesFlags alters flags for the specified message(s).
//
// If the Backend implements Updater, it must notify the client immediately
// via a message update.
func (s *IMAPMailbox) UpdateMessagesFlags(uid bool, seqset *imap.SeqSet, operation imap.FlagsOp, flags []string) error
...
// If the Backend implements Updater, it must notify the client immediately
// via a mailbox update.
func (s *IMAPMailbox) CopyMessages(uid bool, seqset *imap.SeqSet, dest string) error
...
func (s *IMAPMailbox) Expunge() error