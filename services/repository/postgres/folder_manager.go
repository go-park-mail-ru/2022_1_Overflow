...
func (c *Database) AddMailToFolderByObject(context context.Context, request *repository_proto.AddMailToFolderByObjectRequest) (*utils_proto.DatabaseResponse, error) {
	...
	if len(mail.Addressee) == 0 {
		...
		err = c.Conn.QueryRow(context, "INSERT INTO overflow.mails(addressee, date, files, sender, text, theme) VALUES ($1, $2, $3, $4, $5, $6) ON CONFLICT(addressee) ON CONSTRAINT mails_addressee_fkey DO UPDATE SET addressee=excluded.addressee RETURNING id;", mail.Addressee, mail.Date, mail.Files, mail.Sender, mail.Text, mail.Theme).Scan(&mailId)
	}
	if err != nil {
		log.Error(err)
		return &utils_proto.DatabaseResponse{
